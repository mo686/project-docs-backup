...
from PyQt5.QtWidgets import (QApplication, QMainWindow, QWidget, QVBoxLayout, QHBoxLayout, QLabel, QTableWidget,
                             QTableWidgetItem, QPushButton, QComboBox, QGroupBox, QGridLayout, QCheckBox, QMessageBox,
                             QProgressBar, QSplitter, QLineEdit, QDialog, QSpinBox, QDoubleSpinBox, QMenu, QAction,
                             QFileDialog, QHeaderView, QInputDialog, QRadioButton, QTextEdit)
from PyQt5.QtCore import Qt, QThread, pyqtSignal, QTimer, QMutex, QMutexLocker, QPoint
...
class DataAcquisitionThread(QThread):
    ...
    data_updated = pyqtSignal(float, list)
    ...
    def collect_data(self):
        ...
        try:
            ...
            self.data_updated.emit(current_time, data)
        except Exception as e:
        ...
        try:
            ...
            cursor.executemany('''
                INSERT INTO voltage_data (timestamp, channel1, channel2, channel3, channel4, channel5, 
                                          ...
                                          channel18)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            ''', [([datetime.datetime.fromtimestamp(timestamp).strftime('%Y-%m-%d %H:%M:%S')[:-3]] + data) for timestamp, data in batch])
            self.db_conn.commit()
...
class VoltageMonitoringApp(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("18路电压采集上位机 (数据库版)优化")
        self.setMinimumSize(1200, 800)

        # ---------------- 关键改动 ----------------
        self.config_file = resource_path("config.json")
        self.db_file = resource_path("voltage_data.db")
        # ----------------------------------------
    ...
    def init_ui(self):
        ...
        # 设置实时电压数据显示窗口的最小高度
        voltage_widget.setMinimumHeight(160)
    ...
    def create_menu_bar(self):
        ...
        # 配置菜单项
        threshold_action = QAction("阈值设置", self)
        threshold_action.triggered.connect(self.configure_thresholds)
        config_menu.addAction(threshold_action)

        channel_action = QAction("通道名称配置", self)
        channel_action.triggered.connect(self.configure_channel_names)
        config_menu.addAction(channel_action)

        # 新增事件显示方式配置
        display_action = QAction("事件显示方式", self)
        display_action.triggered.connect(self.configure_event_display)
        config_menu.addAction(display_action)

        # 新增图表显示模式配置
        plot_mode_action = QAction("图表显示模式", self)
        plot_mode_action.triggered.connect(self.configure_plot_mode)
        config_menu.addAction(plot_mode_action)

        time_format_action = QAction("切换时间格式", self)
        time_format_action.triggered.connect(self.toggle_time_format)
        config_menu.addAction(time_format_action)
    ...
    def toggle_connection(self):
        ...
        if self.modbus_client.is_connected:
            ...
            if self.modbus_client.connect():
                ...
                self.statusBar().showMessage(f"已连接到 {selected_port}, 波特率 {self.modbus_client.baudrate}")
                self.modbus_client.perform_self_check()
            else:
    ...
    def update_sampling_interval(self, value):
        ...
        self.statusBar().showMessage(f"采样间隔已更新为 {value} ms")
    ...
    def start_storage_thread(self):
        """启动数据存储线程"""
        if not self.storage_thread or not self.storage_thread.isRunning():
    ...
    def save_config(self):
        try:
            ...
            self.time_format = cfg.get("time_format", "seconds")
    ...
    def closeEvent(self, event):
        ...
        if self.storage_thread and self.storage_thread.isRunning():
        ...
        if self.plot_mode == 1:
            ...
            if self.plot_widget.sceneBoundingRect().contains(pos):
                ...
                try:
                    ...
                    self.statusBar().showMessage(f"时间: {date_time_str}, 电压: {voltage:.3f} V")
                except (OSError, ValueError, OverflowError):
                    self.statusBar().showMessage("无效时间戳")
            else:
                self.statusBar().showMessage("就绪")
