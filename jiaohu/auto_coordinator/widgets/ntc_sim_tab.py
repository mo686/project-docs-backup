...
# -*- coding: utf-8 -*-
"""
6 通道 NTC 模拟器 UI（QWidget 子类，适配 QTabWidget）
从 NTC.py 的 MainWindow 适配而来。
"""
import json
import os
import sys
...
CFG_EXCEL_FILE = BASE_DIR / 'NTC.xlsx'
CFG_JSON_FILE = BASE_DIR / 'NTC.json'
...
# ---------- NTC 模拟器 Tab ----------
class NTCSimTab(QWidget):
    data_changed = pyqtSignal(int, float)

    def __init__(self, curves=None, parent=None):
    ...
    def save_cfg(self):
        ...
        try:
            existing = json.loads(CFG_JSON_FILE.read_text(encoding='utf-8'))
        except Exception:
            existing = {}
        for key in ('_heartbeat', '_mode', '_external_command'):
            if key in existing:
                self.cfg[key] = existing[key]
        try:
            tmp = CFG_JSON_FILE.with_suffix('.tmp')
            tmp.write_text(json.dumps(self.cfg, indent=2, ensure_ascii=False), encoding='utf-8')
    ...
    def init_ui(self):
        ...
        self.chk_always_on_top.setVisible(False)
        ...
        btn_load.setObjectName("btn_info")
        ...
        for ch in range(1, 7):
            ...
            le_name.setMinimumWidth(70)
            le_name.setMaximumWidth(90)
            ...
            sp_limit_high.setMinimumWidth(90)
            ...
            sp_limit_low.setMinimumWidth(90)
    ...
    def on_slider_changed(self, idx):
        ...
        self.data_changed.emit(idx, temp_value)
    ...
    def auto_ramp(self):
        ...
        for idx in range(6):
            ...
            self._update_ui_from_data(idx)
            self.update_output_immediately(idx)
            self.data_changed.emit(idx, new_temp)
